...
type GitHubContentsApiResponse = Array<{
  path?: string;
  type?: string;
  size?: number;
}>;
...
function parseGitHubRepositoryUrl(
  ...
  try {
    ...
    return {
      owner: pathParts[0],
      repo: pathParts[1].replace(/\.git$/u, ""),
    };
  } catch {
    return null;
  }
...
const repositoryApiResponseSchema: z.ZodType<GitHubRepositoryApiResponse> = z
  .object({
...
const repositoryContentsApiResponseSchema: z.ZodType<GitHubContentsApiResponse> =
  z.array(
    z
      .object({
